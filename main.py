...
class CLIPImageEncoder(nn.Module):
    def __init__(self, n_embedding_space: int = 512, temperature: float = 0.07):
...
class CLIPTextEncoder(nn.Module):
    def __init__(self, tokenizer: GPT2Tokenizer, n_embedding_space: int = 512, n_vocab: int = 50257):
    ...
    def forward(self, texts):
        # extract feature representations of each modality
        text_inputs = self.bpe(texts, padding=True, return_tensors="pt").to('cuda:0')
...
if __name__ == '__main__':
    ...
    for e in range(args.epoch_from, n_epochs):
        ...
        for images, label in train_tq:
            ...
            # matrix_labels = torch.arange(images.size(0)).to(device)
